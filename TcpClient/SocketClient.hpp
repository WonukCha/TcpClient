...
public:
	...
	virtual void OnRecive(RingbufferMutexLock* receive, size_t size) {};
	...
	bool SendToData(const void* pData, size_t size)
	{
		...
		if (IsConnect())
		{
			mSendBufferLock.PutData(pData, size);
			bResult = true;
		}
		return bResult;
	}
...
protected:
	...
	void ReceiveThread()
	{
		...
		while (true)
		{
			...
			if (receiveSize > 0)
			{
				//Receive Data
				bool bSuccess = false;
				bSuccess = mReceiveBufferLock.PutData(buf, receiveSize);
				OnRecive(&mReceiveBufferLock, receiveSize);
				if (bSuccess == false)
				{
	...
	void SendThread()
	{
		...
		while (true)
		{
			...
			if (!mSendBufferLock.IsEmpty() && IsConnect())
			{
				if (mSendBufferLock.GetSize() > bufferSize)
					sendBufferSize = bufferSize;
				else
					sendBufferSize = mSendBufferLock.GetSize();

				mSendBufferLock.GetData(buf, sendBufferSize);
	...
	RingbufferMutexLock mReceiveBufferLock;
	RingbufferMutexLock mSendBufferLock;
